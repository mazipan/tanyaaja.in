...
import { EnvelopeOpenIcon, ArrowTopRightIcon } from "@radix-ui/react-icons";
import Link from "next/link";
import { useEffect, useState } from "react";

export default function EksplorPage() {
  const [isLoading, setIsLoading] = useState<boolean>(true);
  const [publicUsers, setPublicUsers] = useState<UserProfile[] | null>(
    null
  );
    ...
    <main className="w-full container py-8">
      <div className="w-full space-y-0.5">
        ...
        <p className="text-muted-foreground">
          Kamu cuma bisa mencari pengguna yang tersedia dan sudah memberikan izin untuk ditampilkan di laman ini
        </p>
      </div>
        ...
        {isLoading ? (
          <>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
              {[1, 2, 3].map(item => (
                <Card key={item} className="flex items-center justify-between space-x-4 p-4">
                  <ProfileAvatar image="https://placehold.co/38x38.png" name="U" size="38" />
                  <div className="w-full space-y-0.5">
              ...
              <>
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-2">
                  {publicUsers.map((up: UserProfile) => (
                    <Card key={up.slug} className="flex items-center justify-between space-x-4 p-4">
